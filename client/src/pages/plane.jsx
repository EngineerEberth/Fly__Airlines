...
const Planes = () => {
    ...
    const fetchPlanes = async () => {
        try {
            const response = await axios.get('http://localhost:4000/api/aviones', { withCredentials: true });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editing) {
                await axios.put(`http://localhost:4000/api/aviones/${formData._id}`, formData, { withCredentials: true });
            } else {
                await axios.post('http://localhost:4000/api/aviones', formData, { withCredentials: true });
            }
            fetchPlanes();
            setFormData({
                matricula: '',
    ...
    const handleDelete = async (id) => {
        try {
            await axios.delete(`http://localhost:4000/api/aviones/${id}`, { withCredentials: true });
            fetchPlanes();
        } catch (error) {
