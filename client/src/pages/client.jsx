...
const Clients = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editing) {
                await axios.put(`http://localhost:4000/api/usuarios/${formData._id}`, formData, { withCredentials: true });
            } else {
                await axios.post('http://localhost:4000/api/usuarios', formData, { withCredentials: true });
            }
            fetchClients();
            setFormData({
                nombreCompleto: '',
    ...
    const handleDelete = async (id) => {
        try {
            await axios.delete(`http://localhost:4000/api/usuarios/${id}`, { withCredentials: true });
            fetchClients();
        } catch (error) {
